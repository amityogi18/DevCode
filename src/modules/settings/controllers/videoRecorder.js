...
export class VideoRecorderController {
    ...
    upload(){ 
        ...
        fileReader.onload = (event) => {
            ...
            _this.saveWelcomeVideos(this.videoFile, true);
        }
        else{
            if(_this.youTubeLink && _this.youTubeLink.length>0){
                ...
                _this.saveWelcomeVideos(_this.videoUrl, false);
            }
            else{  
                _this.$timeout(function(){
                    _this.saveWelcomeVideos(_this.recordedBlob, true);   
                },500);                
            };       
        }
    }
   ...
   saveWelcomeVideos(data, isVideo){
       if(_this.youTubeLink){
        ...
        _this.AdminCompanyInfoService.saveWelcomeVideos(data, isVideo).then(()=>{
        _this.getSaveVideos();
        _this.hideBtn = false;
          _this.GrowlerService.growl({
              type: 'success',
    ...
    startRecording(stream) {
        ...
        // this.videoElement = document.getElementById('videoRecorderElement');
        // this.videoElement.src = url ? url.createObjectURL(stream) : stream;
        // this.videoElement.controls = true;
        // this.videoElement.play();

        var mediarecorderoptions =  _this.mediaRecorderService.initializeStartMediaRecorder(stream, 'videoRecorderElement');
    ...
    getSaveVideos(){
            ...
            if(response && response.data.length > 0){
                ...
                if(videoPath){
                    ...
                    if(path.indexOf('youtube') >= 0){
                        ...
                        _this.recordingOn = true;
                        _this.videoElement = document.getElementById('videoRecorderElement');
        ...
        _this.AdminCompanyInfoService.getSaveVideos();
    ...
    getWelcomeVideos(fileId) {
        let onSuccess = (response) => {
            if (response && response.data && response.data.file) {
              ...
              _this.welcomeVideoUpdated(_this.welcomeVideo);
            }
            else {
              _this.welcomeVideo.videoPath = "";
            }
